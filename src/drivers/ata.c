...
ata_status_t ata_write28(ata_device_t *dev, uint32_t lba,
    ...
    port_byte_out(dev->io_base + ATA_REG_COMMAND, ATA_CMD_WRITE_PIO);
    ...
    for (int s = 0; s < sector_count; s++) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            port_word_out(dev->io_base + ATA_REG_DATA, data);
            buffer += 2;
        }
    }

    port_byte_out(dev->io_base + ATA_REG_COMMAND, ATA_CMD_CACHE_FLUSH);
    ata_poll(dev->io_base);
